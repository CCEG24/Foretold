...
Serves ./dist with the cross-origin-isolation headers (COOP/COEP) that WebAssembly
needs and the correct application/wasm MIME — the two things Python's default
http.server doesn't do. No Node required.

    python3 serve.py            # serves ./dist on http://localhost:8000
    python3 serve.py 8080 dist  # custom port / directory
"""
import sys
